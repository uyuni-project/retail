...
def _luks_open(device, luks_pass = None):
    if luks_pass is not None:
        ...
        if res:
            ...
            return luks_device
        res = __salt__['cmd.run_all']("echo {0} | cryptsetup luksOpen {1} {2}".format(luks_pass, device, luks_name), python_shell=True)
...
def _luks_create(device, luks_pass):
    res = __salt__['cmd.run_all']("echo {0} | cryptsetup luksFormat {1}".format(luks_pass, device), python_shell=True)
...
def _check_terminal_kernel_parameters(parameters):
    ...
    kernel_cmdline = str(__salt__['cp.get_file_str']('/proc/cmdline')).split()
